	...
	args = {
		commands = {
			...
			args = {
				commands_help = {
					type = "description",
					name = "/sellomatic -- "..L["this menu."].."\n"..
						"/som -- "..L["same as /sellomatic."].."\n"..
						"/som additem -- "..L["opens add item box."].."\n"..
						"/som delitem -- "..L["opens del item box."].."\n"..
						"/som showlist -- "..L["prints sell/save list if used."].."\n"..
						"/som addtext #text -- "..L["add #text to the sell list if used."].."\n"..
						"/som deltext #text -- "..L["delete #text from the sell list if used."].."\n"..
						"/som importselllist -- "..L["imports old sell list if available."].."\n"..
						"/som resetselllist -- "..L["deletes all sell list data."].."\n"..
						"/som resetsavelist -- "..L["deletes all save list data."]
				},
			},
		},
		-- list_tips = {
		...
		for bag = 0,4,1 do
			for slot = 1, GetContainerNumSlots(bag),1 do
				...
				if name then
					...
					-- Sell epic items
					if self.db.profile.allowEpic and itemRarity == 4 then
						num = self:SellItemList(name,bag,slot,num);
					end;
					-- Sell blue items
					if self.db.profile.allowBlue and itemRarity == 3 then
						num = self:SellItemList(name,bag,slot,num);
					end;
					-- Sell soulbound items
					if self.db.profile.allowBoP and itemRarity == 2 then
						num = self:SellBindItems(ITEM_SOULBOUND,num,name,bag,slot);
					end;
					-- Sell bind on equip items
					if self.db.profile.allowBoE and itemRarity == 2 then
					...
					-- Sell common items
					if self.db.profile.allowWhite and itemRarity == 1 then
...
function SellOMatic:SellItemList(name,bag,slot,num)
	for i,v in ipairs(self.db.profile.sellList) do
		if name and string.find(name,self.db.profile.sellList[i]) or name == v then
...
function SellOMatic:SellItemCheck(name,bag,slot,num)
	...
	for i,v in ipairs(self.db.profile.saveList) do
		if name and string.find(name,self.db.profile.saveList[i]) or name == v then
			save = 1;
		end;
	end;
	if save == 0 then
		...
		for sBag = 0,4,1 do
			for sSlot = 1,GetContainerNumSlots(sBag),1 do
				...
				if sLink then
					...
					if iRarity == 2 and item_tooltip:Find(item_type,1,3) then
