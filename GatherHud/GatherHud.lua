...
--local GetUnitPitch, SaveView = GetUnitPitch, SaveView
...
update:SetScript("OnUpdate", function(self, elapsed)
	...
	-- pitch
	if db.settings.hud_perspective then
		SaveView(5)
		pitch = tonumber(GetCVar("cameraPitchD"))
	--elseif db.settings.use_char_pitch then
...
function GatherHud:IterativeUpdate(force)
	...
	if not force then
		...
		if ((last_x-cx)*zone_w)^2+((last_y-cy)*zone_h)^2 < 2^2 and facing == last_facing then
			--if not (db.settings.hud_perspective or db.settings.use_char_pitch) or pitch == last_pitch then
			if not db.settings.hud_perspective or pitch == last_pitch then
				return
			end
		end
	end
	...
	local y_compression
	--if settings.hud_perspective or settings.use_char_pitch then
	if settings.hud_perspective then
	...
	args = {
		...
		hud_group = {
			...
			args = {
				...
				hud_perspective = {
					name = L["Use Camera Pitch"], type = "toggle",
					desc = L["Use the camera pitch instead of a user-specified angle"],
					arg = "hud_perspective",
					set = function(info, v)
						db.settings.hud_perspective = v
						--if v then
						--	db.settings.use_char_pitch = false
						--end
						GatherHud:IterativeUpdate(true)
						GatherHud:DrawVisualBorder()
						GatherHud:DrawSelf()
				...
				--[[use_char_pitch = {
					...
					set = function(info, v)
						...
						if v then
							db.settings.hud_perspective = false
						end
						GatherHud:IterativeUpdate(true)
						GatherHud:DrawVisualBorder()
						GatherHud:DrawSelf()
				...
				angleofview = {
					...
					args = {
						use_min_pitch = {
							...
							disabled = function() return not db.settings.hud_perspective or not GatherHud:IsEnabled() end,
							order = 1,
						},
						min_pitch = {
							...
							disabled = function() return not db.settings.hud_perspective or not db.settings.use_min_pitch or not GatherHud:IsEnabled() end,
							order = 2,
						},
						y_compression = {
							...
							disabled = function() return db.settings.hud_perspective or not GatherHud:IsEnabled() end,
							order = 3,
						},
					},
				},
